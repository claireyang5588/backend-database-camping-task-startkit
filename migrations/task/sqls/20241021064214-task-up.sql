...
-- 1. 所有教練都有 `重訓` 專長
(
(SELECT c.id FROM "USER" u inner join "COACH" c on u.id=c.user_id 
where u.name='李燕容')
,(SELECT id FROM "SKILL" s WHERE name='重訓')
),
(
(SELECT c.id FROM "USER" u inner join "COACH" c on u.id=c.user_id 
where u.name='肌肉棒子')
...
insert into "COACH_LINK_SKILL" (coach_id , skill_id) values
(
(SELECT c.id FROM "USER" u inner join "COACH" c on u.id=c.user_id where u.name='Q太郎')
,(SELECT id FROM "SKILL" s WHERE name='有氧運動')
),
(
(SELECT c.id FROM "USER" u inner join "COACH" c on u.id=c.user_id where u.name='Q太郎')
...
( -- 1. 教練設定為用戶`李燕容` 
( select u.id from "USER" u inner join "COACH"c on u.id=c.user_id where u.name='李燕容'),
-- 2. 在課程專長 `skill_id` 上設定為「 `重訓` 」
-- 3. 在課程名稱上，設定為「`重訓基礎課`」
-- 4. 授課開始時間`start_at`設定為2024-11-25 14:00:00
-- 5. 授課結束時間`end_at`設定為2024-11-25 16:00:00
-- 6. 最大授課人數`max_participants` 設定為10
-- 7. 授課連結設定`meeting_url`為 https://test-meeting.test.io
(SELECT id FROM "SKILL" s WHERE name='重訓'),'重訓基礎課','2024-11-25 14:00:00','2024-11-25 16:00:00',10,'https://test-meeting.test.io'
)
